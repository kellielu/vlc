    ...
    let higher_order_function_decl_string = 
      ...
      | "map" ->  (generate_variable_type hof.return_array_info.variable_type) ^ 
                Utils.idtos hof.higher_order_function_name ^ "(...)" ^ "{\n\n" ^ 
                      ...
                      "*dimensions = 1;\n" ^ 
                      "return VLC_Array<" ^ (generate_pure_data_type hof.return_array_info.variable_type) ^ ">( (size_t)" ^ string_of_int (hof.array_length) ^ "," ^ Utils.idtos hof.return_array_info.host_name ^ ", (size_t) 1, (size_t *) dimensions" ^ ");\n" ^  
                  "}\n\n\n" 
