...
The first parameter in a `map` or `reduce` function must be a `defg`. For the remaining parameters, `reduce` takes in only one array as the second input, but `map` may take a variable number of arrays so long as they are the same length. All input arrays may not be `null`. 
